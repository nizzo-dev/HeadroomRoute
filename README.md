...
运行 `dist\HeadroomRoute.exe`。首次启动会自动发现组件、备份并配置 Codex 与 Claude Code，然后驻留通知区域。
...
需要 Rust stable。运行 `Build.ps1`，产物位于 `dist\HeadroomRoute.exe`。已安装 Visual Studio C++ Build Tools 时使用系统工具链；开发机没有 SDK 时脚本也支持 cargo-xwin 缓存。
