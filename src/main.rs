...
fn run() -> Result<()> {
    ...
    if mutex.is_null() { anyhow::bail!("无法创建单实例锁"); }
    ...
    if args.iter().any(|arg| arg == "--configure" || arg == "--configure-all") {
    ...
    let (state_dir, legacy_dir) = { let state = app.inner.lock().unwrap(); (state.config.state_dir.clone(), state.config.legacy_state_dir.clone()) };
    ...
    let tray_result = if let Some(seconds) = args.iter().find_map(|arg| arg.strip_prefix("--headless-seconds=").and_then(|v| v.parse::<u64>().ok())) {
