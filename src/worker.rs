use crate::{config, runtime, state::{AppState, should_stop}};
use reqwest::blocking::Client;
use serde_json::Value;
use std::{fs::OpenOptions, process::{Child, Command, Stdio}, sync::{Arc, atomic::Ordering}, thread, time::{Duration, Instant}};

const PROBE_INTERVAL: Duration = Duration::from_secs(60);

pub fn start(app: Arc<AppState>) -> Vec<thread::JoinHandle<()>> {
    vec![probe_loop(app.clone()), headroom_loop(app.clone()), status_loop(app)]
}

fn probe_loop(app: Arc<AppState>) -> thread::JoinHandle<()> {
    thread::spawn(move || {
        let runtime_config = app.inner.lock().unwrap().config.clone();
        let mut client_builder = Client::builder().connect_timeout(Duration::from_secs(3)).timeout(Duration::from_secs(8));
        if let Ok(Some(proxy)) = config::reqwest_outbound_proxy(&runtime_config) { client_builder = client_builder.proxy(proxy); }
...
fn wait_for_next_probe(app: &AppState) {
    ...
    for _ in 0..chunks {
        if should_stop(app) || app.force_probe.swap(false, Ordering::Relaxed) { break; }
            ...
            if !app.sync_in_progress.load(Ordering::Acquire) {
                ...
                if config::routing_drifted(&cfg) {
                    ...
                    if let Err(error) = config::sync_all(&cfg, preferred.as_deref()) {
                        app.inner.lock().unwrap().last_error = Some(format!("CLI 路由守护失败: {error}"));
                    }
                }
            }
...
fn headroom_loop(app: Arc<AppState>) -> thread::JoinHandle<()> {
    thread::spawn(move || {
        ...
        match runtime::ensure_runtime(&initial_config, |status| app.inner.lock().unwrap().headroom_state = status.into()) {
            Ok(python) => {
                let (saved, path) = {
                    let mut state = app.inner.lock().unwrap();
                    state.config.headroom_python = Some(python);
                    state.headroom_state = "运行环境就绪".into();
                    (state.config.clone(), state.config.state_dir.join("config.json"))
                };
                if let Err(error) = config::save(&path, &saved) { app.inner.lock().unwrap().last_error = Some(format!("保存运行环境配置失败: {error}")); }
                let preferred = app.active_url();
                if let Err(error) = config::sync_all(&saved, preferred.as_deref()) { app.inner.lock().unwrap().last_error = Some(format!("运行环境就绪后同步失败: {error}")); }
            }
            Err(error) => {
                let mut state = app.inner.lock().unwrap(); state.headroom_state = "runtime-unavailable".into(); state.last_error = Some(format!("Headroom 运行环境准备失败: {error}"));
                return;
            }
        }
            ...
            if restart {
                ...
                if let Some(mut running) = child.take() { let _ = running.kill(); let _ = running.wait(); }
                    ...
                    if let Some(pid) = listener_pid(port) {
                        ...
                        for _ in 0..20 {
                            if client.get(format!("http://127.0.0.1:{port}/livez")).send().is_err() { break; }
        ...
        if let Some(mut running) = child { let _ = running.kill(); let _ = running.wait(); }
    })
}

fn spawn_headroom(app: &AppState) -> anyhow::Result<Child> {
    let state = app.inner.lock().unwrap();
    let python = state.config.headroom_python.as_ref().ok_or_else(|| anyhow::anyhow!("未找到 Headroom Python 运行时"))?;
    if !python.exists() { anyhow::bail!("运行时不存在: {}", python.display()); }
    std::fs::create_dir_all(&state.config.state_dir)?;
    let stdout = OpenOptions::new().create(true).append(true).open(state.config.state_dir.join("headroom.stdout.log"))?;
    let stderr = OpenOptions::new().create(true).append(true).open(state.config.state_dir.join("headroom.stderr.log"))?;
...
fn sleep_interruptible(app: &AppState, duration: Duration) {
    ...
    for _ in 0..chunks { if should_stop(app) { break; } thread::sleep(Duration::from_millis(100)); }
}

fn listener_pid(port: u16) -> Option<u32> {
    #[cfg(windows)] use std::os::windows::process::CommandExt;
    let output = Command::new("netstat.exe").args(["-ano", "-p", "tcp"]).creation_flags(0x08000000).output().ok()?;
...
fn terminate_pid(pid: u32) -> std::io::Result<()> {
    #[cfg(windows)] use std::os::windows::process::CommandExt;
    let status = Command::new("taskkill.exe").args(["/PID", &pid.to_string(), "/T", "/F"]).creation_flags(0x08000000).status()?;
